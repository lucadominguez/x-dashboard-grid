...
def banner(c):
    p=path('M64 62 H1376',INK,2)+text(64,105,c['eyebrow'].upper(),20,ACCENT,600)
    titles=c.get('title_lines') or textwrap.wrap(c['title'],18)
    assert len(titles)<=3
    ...
    p+=path('M64 625 H1376',LINE,1)+text(64,655,c['footer'].split('/')[0].strip(),19,MUTED)
...
def overview(c):
    ...
    for x in (482,975):p+=poly([(x-7,302),(x+7,309),(x-7,316)],ACCENT)
    for i,node in enumerate(c['nodes']):
        ...
        p+=lines(x,522,node['lines'],23,MUTED,31,'middle')
        p+=text(x,607,str(i+1).zfill(2),20,ACCENT,500,'middle')
    p+=path('M64 648 H1376',LINE,1)+text(64,688,c['caveat'],22,MUTED)
