...
*   **Free / Basic:** per-product writes (100 posts/24h as of the free tier for
    ...
    **sampled** streams (search results ~0.1–3% of real volume). Long-form or
...
2.  The free tier cannot deliver GridX's goal: a full, chronological,
...
4.  No read path gives the user their own timeline affordably.

**Rate-limit reality (as of 2026):** free/Basic read limits are low and sampled;
paid tiers scale but cost money; all API reads are OAuth-guarded and
...
*   **Re-rendering already-loaded DOM.** GridX re-flows DOM nodes X (the
    browser) has already rendered — it is a local presentation layer (a CSS
    grid applied to X's own timeline container), not a data pipeline.
...
**Live-testing note (why v0.2 re-flows instead of hoisting).** The first
implementation, faithful to an earlier design, *moved* every `<article>` out of
X's timeline into a custom grid. On real x.com that emptied the timeline
container, which put X's infinite-scroll sentinel permanently "in view": X then
fired page-after-page of loads with no human scroll cadence, producing
temporary **rate-limits/blocks** and, because we overrode article internals,
**overlapping icons**. v0.2 keeps every article in X's container and columnizes
it in place with CSS (`display:grid; grid-template-columns: repeat(N,…)`). X
keeps ownership of pagination and scroll, so requests follow the user's real
cadence — the strongest possible "no mass fetch / human cadence" posture.
...
1.  **Real risk — X changes its DOM.** `data-testid` selectors and the primary
    ...
    never breaks the page (and restores X exactly on toggle-off).
...
user's own session; no auto-scroll/auto-fetch; fail-safe overlay + clean restore
on toggle-off (`html.gridx-active` class removed, the host's inline grid styles
reverted so X is left byte-for-byte as it was).
...
*   **Storage limits.** `chrome.storage.sync` is ~8KB per item (too small for
    `extraCss`); GridX uses **`chrome.storage.local`** exactly as the brief
    requires.
*   **Content-script world.** GridX runs in the **isolated world**. That means
    our JS globals never collide with the page. Styling targets only **our own
    chrome** (`#gridx-root` overlay: filter bar / keymap / fatal / status) and a
    minimal set of `.gx-stream` grid props on X's container. Critically, we do
    **not** override X's per-article internals (v0.1 tried; it produced
    overlapping icons), so an X markup drift breaks as little as possible and
    article rendering stays native. Where we must win we use `!important`,
    always gated behind `html.gridx-active` so switching off restores X exactly.
...
| X houses the source of truth | Re-flow X's own container in place; never move/remove nodes | Host-detection selectors need upkeep |
| X paginates by its sentinel | Keep articles in X's container so its sentinel stays real (fixes v0.1 rate-limits) | Sentinel/container markup drift |
| X styles fight our grid | Minimal `.gx-stream` grid props + `!important` only where needed | `!important` still needed for chrome hide |
| JS in isolated world | Never touch X globals; style only our chrome | — |
| Playwright fixture proxying | localhost/127.0.0.1 content-script match | Dev-only surface in the manifest |
| Must never break X | Fail-safe overlay + exact restore on toggle-off | — |
